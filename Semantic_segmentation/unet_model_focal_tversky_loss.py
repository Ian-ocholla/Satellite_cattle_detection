...
NUMBER_EPOCHS = 15  # Modify as needed
#BATCH_SIZE = 16      # Modify as needed
DROP_OUT = 0
...
# Use TensorFlow's MirroredStrategy for multiple GPUs
...
#delta= 0.7 #least should be 0.7 to ensure beta is greater than alpha (used as delta) change gamma from 4/3
def focal_tversky_loss(delta=0.3, gamma=4/3):  
    """Focal Tversky loss function for segmentation.
    Reference: https://arxiv.org/abs/1810.07842
    Parameters
    ----------
    delta : float, optional
        Weight for false negatives (FN), default 0.7
    gamma : float, optional
        Controls degree of down-weighting easy examples, default 1.0
    """
    def loss_function(y_true, y_pred):
        epsilon = K.epsilon()

        # Automatically determine axis based on tensor shape
        ...
        # Focal Tversky loss
        loss = K.mean(K.pow((1 - tversky_class), gamma))
...
def unet(pretrained_weights=None, input_size=(PATCHSIZE, PATCHSIZE, NUMBER_BANDS), 
    ...
    with strategy.scope():
        model.compile(optimizer=OPTIMIZER, loss=focal_tversky_loss(delta=0.3, gamma=4/3), metrics=[accuracy, precision_m, recall_m, f1_m]) 
...
weight_path = os.path.join(Folder, "tmp/Lumo_PNEO_FTL_4_3_b8_LM/checkpoint")
...
for fold_no in range(1, num_folds + 1):
    ...
    log_dir = os.path.join(Folder, f"tmp/Lumo_PNEO_FTL_4_3_b8_LM/logs/UNet/fold_{fold_no}_{int(time.time())}")
    os.makedirs(log_dir, exist_ok=True)
    
    
    # Build and sumarize model
    with strategy.scope():
        model = unet(pretrained_weights=None, input_size=(PATCHSIZE, PATCHSIZE, NUMBER_BANDS), lr=LEARNING_RATE, drop_out=DROP_OUT)
    
    # Define callbacks
    
    callbacks_list = [
        ModelCheckpoint(
            filepath=os.path.join(Folder, 'tmp/Lumo_PNEO_FTL_4_3_b8_LM/checkpoint', f"fold_{fold_no}_weights.{{epoch:02d}}-{{val_loss:.4f}}.weights.h5"),
