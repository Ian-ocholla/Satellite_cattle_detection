...
def unet(input_shape=(PATCHSIZE, PATCHSIZE, NUMBER_BANDS), lr=LEARNING_RATE, drop_out=DROP_OUT):
    ...
    with strategy.scope():
        model.compile(optimizer=optimizer,
                      loss=focal_tversky_loss(delta=0.3, gamma=4/3), #(alpha, beta),
...
weight_path = os.path.join(Folder, "tmp/Lumo_PNEO_ResNet_FTL/checkpoint")
...
for fold_no in range(1, num_folds + 1):
    ...
    log_dir = os.path.join(Folder, f"tmp/Lumo_PNEO_ResNet_FTL/logs/UNet/fold_{fold_no}_{int(time.time())}")
    ...
    callbacks_list = [
        ModelCheckpoint(
            filepath=os.path.join(Folder, 'tmp/Lumo_PNEO_ResNet_FTL/checkpoint', f"fold_{fold_no}_weights.{{epoch:02d}}-{{val_loss:.4f}}.weights.h5"),
