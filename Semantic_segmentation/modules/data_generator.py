...
class DataGenerator(object):
  def __init__(self, image_path, label_path, test_image_path, test_label_path, input_image_channel=[1,2,3], patchsize=336): #336
  ...
  def load_image(self, image_path):
    ...
    for f in sorted(os.listdir(image_path)):
      ...
      if ext.lower() == ".tif": #.jpg or .tif
          ...
          img_array = np.stack(bands, axis=2)
          #Normalize the image data to [0,255]
          img_array = img_array /np.max(img_array)*255 #scale to [0,255]
          image_list[imgNo] = np.uint8(img_array) #convert to uint8
    return image_list
